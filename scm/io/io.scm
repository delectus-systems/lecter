    ...
    (let ((head (if (null? rowvals) #f (car rowvals)))
      ...
      (for-each (lambda (it)
                  ...
                  (cond ((eqv? it #f)(display "false"))
                        ((eqv? it ':FALSE)(display "false"))
                        ((eqv? it ':TRUE)(display "true"))
                        ((eqv? it #t)(display "true"))
                        (else (write it))))
                tail))
    (display "]")))

(define (write-delectus-json path)
  (let* ((raw (io:read-binary-file path))
         (created (io:file-creation-time path))
         (modified (io:file-modification-time path))
         (data (u8vector->object raw))
...
;;; (define $movies-path "/Users/mikel/Workshop/src/delectus/delectus-testdata/Movies.delectus")
;;; (define $movies (delectus->lisp $movies-path))


(define (canonicalize-for-json val)
  (cond ((equal? val ':FALSE) 'false)
        (else val)))
...
(define (write-jsonl path)
  (let* ((raw (io:read-binary-file path))
         ...
         (columns-tail (member ':COLUMNS lispdata))
         (columns (if columns-tail
                      (map cadr (cadr columns-tail))
                      '()))
         (rows-tail (member ':ROWS lispdata))
         (rows (if (and rows-tail (not (null? rows-tail)))
                   (cadr rows-tail)
                   '()))
         (row-values (map (lambda (r)
                            (let ((entries-tail (member ':ENTRIES r)))
                              (if (and entries-tail (not (null? entries-tail)))
                                  (cadr entries-tail)
                                  '())))
                          rows)))
    (for-each (lambda (vals)
                (newline)
                (display "{")
                (if (and columns (not (null? columns)))
                    (let ((first-column (car columns))
                          (rest-columns (cdr columns)))
                      (write first-column)(display ": ")(write (car vals))
                      (if (and rest-columns (not (null? rest-columns)))
                          (map (lambda (c v)
                                 (let ((c (canonicalize-for-json c))
                                       (v (canonicalize-for-json v)))
                                   (display ", ")(write c)(display ": ")(write v)))
                               rest-columns (cdr vals))
                          #f))
                    #f)
                (display "}"))
              row-values)
